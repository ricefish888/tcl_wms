...
public class DefaultTclRfOrderManager extends DefaultBaseManager implements TclRfOrderManager{
	...
	public Map orderCodeInputCommit(Map orderMap) throws RfBusinessException {
		...
		if(order==null){
			  ...
			  if(num==null||num<1){
				  orderMap.put("orderCodeInput", "");
				  throw new RfBusinessException("生产工单/线体号不存在",orderMap);
			  }
	...
	public Map orderItemCommit(Map orderMap) throws RfBusinessException {
		...
		if(!WmsBarCodeParse.isBarCode(barCode)){
			orderMap.put("itemInput", "");
			throw new RfBusinessException("条码格式不正确",orderMap);
		}
		...
		if(count==null||count<1){
			orderMap.put("itemInput", "");
			throw new RfBusinessException("无此物料",orderMap);
		} 
	...
	public Map itemBackInfo(Map orderMap) throws RfBusinessException {
		...
		if(WmsBarCodeParse.isBarCode(barCode)){
			...
			lotInfo = map.get(WmsBarCodeParse.KEY_LOTNO).toString().trim();
			item = this.getWmsItem(itemCode);
			if(null==item){
				orderMap.put("itemBackInput", "");
				throw new RfBusinessException("货品不存在!!",orderMap);
			}
		}else{
			itemCode = barCode;
			item = this.getWmsItem(itemCode);
			...
			if(WmsItemScanCode.SCANCODE_NO.equals(item.getUserFieldV10())){
				orderMap.put("itemBackInput", "");
				throw new RfBusinessException("请扫描条码",orderMap);
			}
		}
		TclRfAsnManager tclRfAsnManager = (TclRfAsnManager) applicationContext.getBean("tclRfAsnManager");
		tclRfAsnManager.checkBarCode(barCode);
		...
		String xmlb = "";
		if(WarehouseEnumeration.VMI.equals(wh.getCode())){
			xmlb = WmsFactoryXmlb.JS;
		}else{
			xmlb = WmsFactoryXmlb.BZ;
		}
		orderMap.put("xmlb", xmlb);
	...
	public Map poInfo(Map orderMap) throws RfBusinessException {
		...
		WmsShippingLotTruck wslt = null;
		WmsItem item = null;
		String barCode = null;
		if(null!=orderMap.get("id")){
			String wsltId = orderMap.get("id").toString().trim();
			if(!StringHelper.isNullOrEmpty(wsltId)){
				wslt = commonDao.load(WmsShippingLotTruck.class, Long.valueOf(wsltId));
			}
		}
		...
		orderMap.put(RfConstant.ENTITY_ID, wslt.getSubRelateId());
		

		
		String hql = "FROM WmsShippingLotTruck t where t.item.code=:barcode  AND t.billType='SCLLD' AND t.subRelateId=:subRelateId " +
				"AND t.lotInfo.extendPropC8=:xmlb AND t.lotInfo.supplierCode=:supplierCode AND t.lotInfo.lot=:lot";
		List<WmsShippingLotTruck> trucks = commonDao.findByQuery(hql,new String[]{"barcode","subRelateId","xmlb","supplierCode","lot"},
				new Object[]{item.getCode(),wslt.getSubRelateId(),wslt.getLotInfo().getExtendPropC8(),wslt.getLotInfo().getSupplierCode(),wslt.getLotInfo().getLot()});
		Double shipqty=0d;
		for(WmsShippingLotTruck truck : trucks) {
			shipqty = shipqty + ( truck.getQty()-truck.getReturnQty());
		}
		orderMap.put("shipQty", CommonHelper.dealDoubleError(shipqty));//已发运数量
		orderMap.put("scanedQty", getScanQty(barCode, item, wslt.getSubRelateId()));//已扫描数量
	...
	private Double getScanQty (String barCode,WmsItem item,Long detailId) {
		...
		//查询已扫描的数量
		String scanHql = "FROM WmsProOrderBackRfRecord record where 1=1 "
				+ " and record.wmsShippingLotTruck.item=:ri and record.wmsShippingLotTruck.subRelateId=:rs and record.userId=:ru";
		List<WmsProOrderBackRfRecord> record =  commonDao.findByQuery(scanHql, new String[]{"ri","rs","ru"}, 
				new Object[]{item,detailId,UserHolder.getUser().getId()});
		Double qty = 0d ;
	...
	private Map confirmOrEdit(Map orderMap,Boolean isEdit) throws RfBusinessException {
		...
		String hql = "FROM WmsShippingLotTruck t where t.item.code=:barcode  AND t.billType='SCLLD' AND t.subRelateId=:subRelateId " +
		"AND t.lotInfo.extendPropC8=:xmlb AND t.lotInfo.supplierCode=:supplierCode AND t.lotInfo.lot=:lot AND t.qty>t.returnQty ";
		List<WmsShippingLotTruck> trucks = commonDao.findByQuery(hql,new String[]{"barcode","subRelateId","xmlb","supplierCode","lot"},
				new Object[]{item.getCode(),wslt.getSubRelateId(),wslt.getLotInfo().getExtendPropC8(),wslt.getLotInfo().getSupplierCode(),wslt.getLotInfo().getLot()});
		...
		String scanHql = "FROM WmsProOrderBackRfRecord record where 1=1 "
				+ " and record.wmsShippingLotTruck.item=:ri and record.wmsShippingLotTruck.subRelateId=:rs and record.userId=:ru";
		List<WmsProOrderBackRfRecord> records =  commonDao.findByQuery(scanHql, new String[]{"ri","rs","ru"}, 
				new Object[]{item,wslt.getSubRelateId(),UserHolder.getUser().getId()});
		...
		if(DoubleUtils.add(qt,scanQty)>CommonHelper.dealDoubleError(shipQty)){
			throw new RfBusinessException("数量与发运数量不符!!");
		}
		Double mustBack = DoubleUtils.add(qt,scanQty);
...
//			poInfoStr.append("操作人:").append(userName).append("\n");
	...
	public Map genASN(Map orderMap) throws RfBusinessException {
		...
		String invStatus = (String) orderMap.get("invStatusInput");
		
		//查询待处理的记录
		List<WmsProOrderBackRfRecord> records = commonDao.findByQuery("FROM WmsProOrderBackRfRecord record where 1=1 and record.userId=:ru","ru",UserHolder.getUser().getId());
		if(records.size()>0){
			//生成ASN
			Set<String> keys = new HashSet<String>();
			for (WmsProOrderBackRfRecord record : records) {
				//工单号 ### 供应商编码 ### 项目类别
				keys.add(record.getWmsShippingLotTruck().getRelateCode()+"###"+record.getWmsShippingLotTruck().getLotInfo().getSupplierCode() +"###"+record.getWmsShippingLotTruck().getLotInfo().getExtendPropC8()+"###"+invStatus);
			}
			for(String key : keys) {
	...
	private WmsASN genHeader(List<WmsProOrderBackRfRecord> temps,String key ) {
		...
		if(null != xmlb && xmlb.equals(WmsFactoryXmlb.JS)){//寄售退VMI,标准退自管仓
			hql = "FROM WmsWarehouse warehouse WHERE warehouse.code='VMI'";
			wh = (WmsWarehouse)commonDao.findByQueryUniqueResult(hql, "", "");
		}else{
			hql = "FROM WmsFactoryWarehouse fw WHERE fw.factory.id =:factoryId AND fw.type =:type";
			WmsFactoryWarehouse wfw = (WmsFactoryWarehouse)commonDao.
					findByQueryUniqueResult(hql, new String[]{"factoryId","type"}, 
							new Object[]{productionOrder.getFactory().getId(),xmlb});
			wh = wfw.getWarehouse();
		}
		asn.setWarehouse(wh);
		...
		asn.setCode(asnCode);
		//谁操作的生产退料,生成的收货单保管员就记录谁
		asn.setKeeper(UserHolder.getUser());
		...
		for (WmsProOrderBackRfRecord record : temps) {
			...
			if(asndetails.isEmpty()) {
		        ...
		        detail.setInventoryStatus(invStatus);//库存状态
		        detail.setUserField2(t.getSubRelateId().toString());//记录工单明细ID
			...
			//生产退料要生成拣货单，工单还是要补单的
			WmsTclPickticketManager wmsTclPickticketManager = (WmsTclPickticketManager) applicationContext.getBean("wmsTclPickticketManager");
			wmsTclPickticketManager.createPickByTj(t.getSubRelateId(), record.getBackQty(), wh,WmsPickSource.RF);
		}
		WmsASNManager wmsASNManager = (WmsASNManager) applicationContext.getBean("wmsASNManager");
	    wmsASNManager.isActiveAsn(asn);
//	    wmsTclAsnManager.refreshDetail(asn);//刷新标签数量  生效时会刷新标签数量
	    wmsTclAsnManager.receiveAll(asn.getId(), UserHolder.getUser().getId());
